...
@interface PhotoBrowser ()<UIScrollViewDelegate,ZoomingScrollViewDelegate>
...
#pragma mark 创建底部组件
- (void)setActionSheetWithTitle:(nullable NSString *)title delegate:(nullable id<PhotoBrowserDelegate>)delegate cancelButtonTitle:(nullable NSString *)cancelButtonTitle deleteButtonTitle:(nullable NSString *)deleteButtonTitle otherButtonTitles:(nullable NSString *)otherButtonTitle, ...
{
...
{
    ZoomingScrollView *currentZoomingScrollView = [self zoomingScrollViewAtIndex:self.currentImageIndex];
    if (longPress.state == UIGestureRecognizerStateBegan) {
        ...
        [actionSheet showWithSelectedCompletion:^(NSInteger selectedIndex) {
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(photoBrowser:clickedActionSheetIndex:currentImageIndex:)]) {
                [weakSelf.delegate photoBrowser:weakSelf clickedActionSheetIndex:selectedIndex currentImageIndex:weakSelf.currentImageIndex];
            }
        }];
    }
}
...
- (void)showPhotos
{
    ...
    for (ZoomingScrollView *zoomingScrollView in self.visibleZoomingScrollViews) {
...
{
    for (ZoomingScrollView* view in self.visibleZoomingScrollViews) {
...
{
    ZoomingScrollView *zoomingScrollView = [self dequeueReusableZoomingScrollView];
    ...
    zoomingScrollView.frame = CGRectMake((self.scrollView.width) * index, 0, self.width, self.height);
    self.currentImageIndex = index;
    if (zoomingScrollView.hasLoadImage == NO) {
        ...
        zoomingScrollView.hasLoadImage = YES;
    }
 ...
 */
- (ZoomingScrollView *)dequeueReusableZoomingScrollView
{
    ZoomingScrollView *photoView = [self.reusableZoomingScrollViews anyObject];
    if (photoView) {
        [self.reusableZoomingScrollViews removeObject:photoView];
    } else {
        photoView = [[ZoomingScrollView alloc] init];
    }
    return photoView;
}
...
- (UIWindow *)findTheMainWindow
{
    ...
    for (UIWindow *window in frontToBackWindows) {
        ...
        BOOL windowSizeIsEqualToScreen = (window.width == SCREEN_WIDTH && window.height == SCREEN_HEIGHT);
        if(windowOnMainScreen && windowIsVisible && windowLevelSupported && windowSizeIsEqualToScreen) {
            return window;
        }
    }
    
    NSLog(@"XLPhotoBrowser在当前工程未匹配到合适的window,请根据工程架构酌情调整此方法,匹配最优窗口");
    if (PhotoBrowserDebug) {
...
- (void)showFirstImage
{
    ...
    ImageCollectionViewCell *cell = (ImageCollectionViewCell *)self.sourceImageView;
    ...
    CGFloat height = SCREEN_WIDTH / imageWidthHeightRatio;
    CGFloat x = 0;
    CGFloat y;
...
{
    if (self.imageCount == 1 && self.hidePageControlOnSingleImage == YES) {
 ...
 *  @param index 指定位置索引
 */
- (ZoomingScrollView *)zoomingScrollViewAtIndex:(NSInteger)index
{
    for (ZoomingScrollView* zoomingScrollView in self.visibleZoomingScrollViews) {
...
- (void)saveImage
{
    ZoomingScrollView *zoomingScrollView = [self zoomingScrollViewAtIndex:self.currentImageIndex];
    if (zoomingScrollView.progress < 1.0) {
        self.savaImageTipLabel.text = PhotoBrowserLoadingImageText;
    ...
    UIImageWriteToSavedPhotosAlbum(zoomingScrollView.currentImage, self, @selector(image:didFinishSavingWithError:contextInfo:), (__bridge void *)self);
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    [self addSubview:self.savaImageTipLabel];
    if (error) {
        self.savaImageTipLabel.text = PhotoBrowserSaveImageFailText;
    } else {
        self.savaImageTipLabel.text = PhotoBrowserSaveImageSuccessText;
    }
    [self.savaImageTipLabel performSelector:@selector(removeFromSuperview) withObject:nil afterDelay:1.0];
}
...
#pragma mark - ZoomingScrollViewDelegate
/**
 *  单击图片,退出浏览
 */
- (void)zoomingScrollView:(ZoomingScrollView *)zoomingScrollView singleTapDetected:(UITapGestureRecognizer *)singleTap
{
    ...
    tempView.frame = CGRectMake( - zoomingScrollView.contentOffset.x + zoomingScrollView.imageView.left,  - zoomingScrollView.contentOffset.y + zoomingScrollView.imageView.top, zoomingScrollView.imageView.width, zoomingScrollView.imageView.height);
    [self addSubview:tempView];
...
- (void)initial
{
    ...
    _pageControlAliment = PhotoBrowserPageControlAlimentCenter;
    _showPageControl = YES;
    _pageControlDotSize = CGSizeMake(10, 10);
    _pageControlStyle = PageControlStyleAnimation;
    _hidePageControlOnSingleImage = YES;
    _currentPageControlColor = [UIColor whiteColor];
    _otherPageControlColor = [UIColor lightGrayColor];
    _browserStyle = PhotoBrowserStylePageControl;
...
- (void)setupScrollView {
    ...
    rect.size.width += PhotoBrowserImageViewMargin;
...
- (void)setUpPageControl
{
    ...
    if ((self.imageCount <= 1) && self.hidePageControlOnSingleImage) {
        return;
    }
    
    switch (self.pageControlStyle) {
        case PageControlStyleAnimation:
        {
            TAPageControl *pageControl = [[TAPageControl alloc] init];
            pageControl.numberOfPages = self.imageCount;
            pageControl.dotColor = self.currentPageControlColor;
            pageControl.currentPage = self.currentImageIndex;
            ...
            break;
        case PageControlStyleClassic:
        {
            UIPageControl *pageControl = [[UIPageControl alloc] init];
            _pageControl = pageControl;
            pageControl.numberOfPages = self.imageCount;
            pageControl.currentPageIndicatorTintColor = self.currentPageControlColor;
            pageControl.pageIndicatorTintColor = self.otherPageControlColor;
            pageControl.userInteractionEnabled = NO;
            [self addSubview:pageControl];
            pageControl.currentPage = self.currentImageIndex;
        }
    ...
    // 重设pagecontroldot图片
    self.currentPageControlImage = self.currentPageControlImage;
    self.otherPageControlImage = self.otherPageControlImage;
    ...
    if (self.pageControlAliment == PhotoBrowserPageControlAlimentRight) {
        x = self.width - size.width - 10;
    }
    CGFloat y = self.height - size.height - 10;
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        TAPageControl *pageControl = (TAPageControl *)_pageControl;
...
- (void)setUpToolBars
{
    ...
    indexLabel.centerX = self.width * 0.5;
    indexLabel.centerY = 35;
...
- (void)setUpBrowserStyle
{
    switch (self.browserStyle) {
        case PhotoBrowserStylePageControl:
        {
            ...
            break;
        case PhotoBrowserStyleIndexLabel:
        {
...
- (void)layoutSubviews
{
    ...
    _savaImageTipLabel.height = 30;
    _savaImageTipLabel.width += 20;
...
- (void)setBrowserStyle:(PhotoBrowserStyle)browserStyle
{
...
{
    _currentPageControlColor = currentPageDotColor;
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        TAPageControl *pageControl = (TAPageControl *)_pageControl;
...
{
    _otherPageControlColor = pageDotColor;
    if ([self.otherPageControlColor isKindOfClass:[UIPageControl class]]) {
        UIPageControl *pageControl = (UIPageControl *)_pageControl;
...
{
    _currentPageControlImage = currentPageDotImage;
    [self setCustomPageControlDotImage:currentPageDotImage isCurrentPageDot:YES];
}

- (void)setPageDotImage:(UIImage *)pageDotImage
{
    _otherPageControlImage = pageDotImage;
...
- (UIImage *)placeholderImage
{
    if (!_placeholderImage) {
        _placeholderImage = [UIImage imageWithColor:[UIColor grayColor] size:CGSizeMake(100, 100)];
    }
